/*
Author: Ampersand editied by Challenger
Check if it's destroy bush or cut grass.

Changelog:
21-05/23: Removed dependency on CBA setting, always requires entrenching tool or IMS Shovel.
...
if
(
	!("ACE_EntrenchingTool" in (items _player)) || !("Shovel_Russian_Rotated" in (handgunWeapon _player)) || !("Shovel_Russian_Rotated" in (items _player)) 
) exitWith {false};
